...
function selectProduct(pk){
  ...
  // 更新 logo
  document.getElementById('logoIcon').src=ALL_DATA[pk].icon;
  document.getElementById('logoName').textContent=ALL_DATA[pk].name;
...
def generate_html(products_list, all_rankings=None):
    ...
    for prod_name, prod_key, data, labels, img_prefix, _, icon_path, _fn in products_list:
        ...
        all_data_js[prod_key] = {
            "name": prod_name, "icon": icon_path, "dates": prod_dates,
            "imgs": prod_imgs,
